...
$(function() {
    ...
    $.fn.elastic_grid = function(G) {
        G = $.extend({}, {items: null, filterEffect: "", hoverDirection: true, hoverDelay: 0, hoverInverse: false, expandingHeight: 500, expandingSpeed: 300, 
        ...
        function l(i) {
            i.on("click", "span.og-close", function() {
                z();
                i.children().find('figure').css('top','100%');
                return false
        ...
        e.prototype = {create: function() {
                ...
                if(G.exControls == true){
                    
                    this.$itemNext = $('<span>'+G.exControlsNextText+'</span>');
                    this.$itemNext.css({right:0});
                    if(G.exControlsCSS != null)
                        this.$itemNext.css(G.exControlsCSS);                    
                    if(G.exControlsNextClass != null)
                        this.$itemNext.addClass(G.exControlsNextClass);                    
                    this.$itemNext.click(function(){
                        var pUl = $(this).parent().parent().parent().parent().children();
                        pUl.find('figure').css('top','100%');
                        var cH = $(this).parent().parent().parent();
                        var cHDataType = cH.data('tags');
                        var cHdIndex = cH.index();
                        pUl.removeClass('animate');

                        if($('#portfolio-filter').find('li[class=current]').children().data('filter') == '.all')
                        {
                            a($(this).parent().parent().parent().next());
                            return;
                        }                        
                        //console.log(cHDataType);
                        var jc=[];
                        for (var i = 0; i < pUl.length; i++) {
                            // console.log(pUl.eq(i).data('tags').search(cHDataType));
                            if(cHDataType.indexOf(',') >= 0)                                          
                            {
                                var chArray = cHDataType.split(',');                                
                                
                                for (var j = 0; j < chArray.length; j++) {                                    
                                    if(pUl.eq(i).data('tags').indexOf(chArray[j]) >= 0)
                                    {                                                                      
                                        if($.inArray(i,jc) < 0)
                                        {
                                            //console.log(i,pUl.eq(i));
                                            jc.push(i);
                                        }
                                    }
                                };
                                ...
                                if(pUl.eq(i).data('tags').indexOf(cHDataType) >= 0)
                                    {                                                                      
                                        //console.log(i,pUl.eq(i));
                                        jc.push(i);
                                    }
                        ...
                        // pUllength = jc;
                        //console.log(jc,$.inArray(cHdIndex,jc)); 
                        
                        pUl.removeClass('animate');                       

                        if(jc.length > 1 && $.inArray(cHdIndex,jc) != jc.length -1)
                        	a(pUl.eq(jc[$.inArray(cHdIndex,jc) + 1]));                        
                    });

                    this.$itemPrev = $('<span>'+G.exControlsPrevText+'</span>');
                    this.$itemPrev.css({left:0});                    
                    if(G.exControlsCSS != null)
                        this.$itemPrev.css(G.exControlsCSS);
                    if(G.exControlsPrevClass != null)
                        this.$itemPrev.addClass(G.exControlsPrevClass);                                        
                    this.$itemPrev.click(function(){
                        ...
                        if($('#portfolio-filter').find('li[class=current]').children().data('filter') == '.all')
                        {
                            a($(this).parent().parent().parent().prev());
                            return;
                        }
                ...
                if (typeof G.items[y] === "undefined") {
                    ...
                    eldata = G.items[y];
                    if(G.exHideInfoBoxTitle == false) //Checking Config
                    ...
                    if (glarge.length == gthumbs.length && glarge.length > 0) {
                        ...
                        I.$details.prepend('<div class="infosep"></div>');
                        I.$details.prepend(M)
                    } else {
